...
│   ├── cli.py               # argparse CLI(`jykj-ocr` / `serve` / `--list-engines` / `--engine` / `--format`)
...
- **FastAPI**:
  ...
  - `POST /ocr`(multipart,文件 + 可选 `engine`/`model`/`prompt`/`strategy`/`max_pages`/`dpi`/`format`)
  - `POST /ocr/text`(image_url 文本请求)
- **Python API**:`jykj_ocr.ocr(source, engine=None, config=None, config_path=None, max_pages=None, dpi=200, retries=1) -> List[OCRResult]`。
...
- **策略引擎**:按顺序尝试每个引擎,支持 `should_retry_no_text` / `should_retry_low_confidence` 判定是否重试。
...
- **`_REMOTE_ENGINES`**:只有 siliconflow/multimodal 响应 `model` / `prompt` 覆盖;本地 rapidocr 不受影响。
